...
def get_smooth_pv(frequencies,corr_spectrum,interstation_distance,ref_curve,
                  ...
                  pick_threshold=2.,std_prior=0.3,
                  horizontal_polarization=False, smooth_spectrum=False,
    ...
    :type pick_threshold: float
    :param pick_threshold: Controls the ratio of maximum to minimum amplitude
        in the heatmap plot. If the ratio is lowe than pick_threshold, no pick
        is taken
    :type std_prior: float
    :param std_prior: The heatmap is weighted with a Gaussian centered at
        the predicted velocity for the next pick. In the beginning, this is the
        reference curve. Later, the last picks and the slope of the already
        picked curve is used. std_prior is in units of km/s. Choosing a large 
        value (>1.0) makes the influence of the weighting negligible.        
    :type horizontal_polarization: bool
    :param horizontal_polarization: When ``True``, the zero crossings from the spectrum are
        compared to the difference function J0 - J2 (Bessel functions of the
        first kind of order 0 and 2 respectively, see Aki 1957). Appropriate for Love- and the
...
#%%
    ...
    def get_kernel(X,Y,freq,vel,slope,interstation_distance,
        ...
        # the height of the kernel ellipse is limited to 1km/s
        ...
        if return_poly_coords:
            ...
            dv_cycle_bound = dv_cycle_jump(fax_bound,v_predicted_bound,
                                           interstation_distance)
            dv_cycle_bound[dv_cycle_bound>1] = 1
    ...
    def update_density_field(X,Y,density,crossings,ref_vel,
        ...
        This weighting is controlled by std_prior. Values larger than 1.0 
        usually result in no significant influence from the weighting procedure.
        """
        ...
        for j,(freq,vel,slope) in enumerate(crossings):
            ...
            poly_coords, poly_weight_ind, poly_weights = get_kernel(
                    ...
                    return_weights=True)
            sig = std_prior # standard deviation in km/s
    ...
    def check_previous_picks(picks,picks_backup,frequency,slope,minvel,maxvel,
        ...
        if len(picks) > 0:
            ...
            if ((total_picks_jumped > no_test_picks and total_picks_jumped/len(picks) > 0.3) or
    ...
    def check_zero_crossing_quality(frequencies,spectrum,zero_crossings,
        ...
        for i in range(1,len(cross_freqs)-1):
            ...
            if 1./cross_freqs[i] > 30.:
                ...
                if new_closest_vel > closest_vel+0.20:
    ...
    while idx_pick <= idx_gridpoints and idx_pick < len(gridaxis):
        ...
        while idx_kernel < len(w_axis):
            ...
            if not bad_quality[idx_kernel]:
                density, ell_paths = update_density_field(
                    X,Y,density,crossings[cross_idx],reference_velocity,
                    filt_width,filt_height,interstation_distance,
                    std_prior=std_prior,idx_plot=idx_plot)
        ...
        if freq_pick>freqmin:
            fstep = reference_curve_func(freq)/(2*interstation_distance)
            bad_crossings = bad_quality[(w_axis>freq_pick-fstep)*(w_axis<freq_pick+fstep)]
            #if (bad_quality[np.abs(w_axis-freq_pick).argsort()][:3]).all():
            if np.sum(bad_crossings)<np.sum(~bad_crossings):
    ...
    if plotting: 
        ...
        if len(corr_spectrum)>1:
            bq = np.isin(crossings[:,0],w_axis[bad_quality])
            ax.plot(crossings[bq,0],crossings[bq,1],'ko',markeredgecolor='black',
                    ms=3,linewidth=0.1)
            ax.plot(crossings[~bq,0],crossings[~bq,1],'wo',markeredgecolor='black',
                    ms=3,linewidth=0.1)
        else:
            for branchidx in np.unique(zero_crossings[:,2]):
                plot_crossings = zero_crossings[zero_crossings[:,2]==branchidx,:]
                bq = np.isin(plot_crossings[:,0],w_axis[bad_quality])
                ax.plot(plot_crossings[bq,0],
                        plot_crossings[bq,1],'ko',
                        markeredgecolor='black',ms=4,linewidth=0.1)
                ax.plot(plot_crossings[~bq,0],
                        plot_crossings[~bq,1],'o',
                        markeredgecolor='black',ms=4,linewidth=0.1)
        ax.plot([],[],'wo',markeredgecolor='black',ms=3,label='crossing')
        ax.plot([],[],'ko',markeredgecolor='black',ms=3,label='crossing (low quality)')
        ...
        plt.show()
    #%%
    if True: # additional instructive plot, should not be used
        #%%
        ...
        cbar = ax.contourf(xplot,yplot,heatmap,vmin=0,vmax=1,
                             levels=20)
        #cbar = ax.scatter(xplot.flatten(),yplot.flatten(),c=heatmap.flatten(),vmin=0,vmax=1)
        for c in cbar.collections:
            c.set_rasterized(True)
        for fi in range(len(crossings)):
            freq = crossings[fi,0]
            vel = crossings[fi,1]
            if freq<=freqwin[0] or freq>=freqwin[1]:
                continue
        #for freq in w_axis[(w_axis>=freqwin[0])*(w_axis<=freqwin[1])]:
            xyA = (freq,0)
            xyB = (freq,vel)#velwin[0])
            con = ConnectionPatch(xyA=xyA, xyB=xyB, coordsA="data", coordsB="data",
                                  axesA=ax0, axesB=ax, color="grey",linestyle='dashed',
                                  linewidth=0.5)
            ax.add_artist(con)
        ax.plot(testfreq,testvel,'ro',ms=8)
        if len(corr_spectrum)>1:
            bq = np.isin(crossings[:,0],w_axis[bad_quality])
            ax.plot(crossings[bq,0],crossings[bq,1],'ko',markeredgecolor='black',
                    ms=3,linewidth=0.1)
        ...
        xline = np.max(boundary_coords[:,0])+(np.max(boundary_coords[:,0])-np.min(boundary_coords[:,0]))*0.05
        ...
        ax0.set_xlim(freqwin[0],freqwin[1])
        ax.set_xlim(freqwin[0],freqwin[1])
        ax2.set_xlim(ax.get_xlim())
        new_tick_locations = ax.get_xticks()[ax.get_xticks()<np.max(ax.get_xlim())]
        ax2.xaxis.set_ticks_position("bottom")
        ax2.xaxis.set_label_position("bottom")
        ...
        #ax0.plot(w_axis,peakamps,'k')
        ax0.plot(frequencies[0],np.zeros(len(frequencies[0])),'k',linewidth=0.3)
        ax0.plot(w_axis[bad_quality],np.zeros(np.sum(bad_quality)),'ko',
                 markersize=2,label='low quality crossing')
        ax0.legend(loc='upper right',framealpha=0.85)
        # zero crossing plot
        ...
        if len(corr_spectrum)>1:
            ...
            for branchidx in np.unique(zero_crossings[:,2]):
                ...
                ax.plot(plot_crossings[~bq,0],
                        plot_crossings[~bq,1],'o',
                        markeredgecolor='black',ms=4,linewidth=0.1)
        ax.plot([],[],'wo',markeredgecolor='black',ms=3,linewidth=0.1,label='crossing')
        ax.plot([],[],'ko',markeredgecolor='black',ms=3,linewidth=0.1,label='crossing (low quality)')
        ax.plot(ref_curve[:,0],ref_curve[:,1],linewidth=2,color='lightblue',label='reference curve')
        #plt.plot(tipx,tipy,'o')
        if False:
            for vertices in ellipse_paths[::1]:
        ...
        for pick in picks:
            ax.plot(pick[0],pick[1],'x',color='orange',ms=5)
        ax.plot([],[],'x',color='orange',ms=5,label='picks')
        ax.plot(smooth_picks_x,smooth_picks,'r--',label='dispersion curve')
        if plotitem is not None:
            ax.plot(plotitem[:,0],plotitem[:,1],'--',color='lightgrey',
                    linewidth=3,label='plotitem')
        # Create a Rectangle patch
        rect = Rectangle((freqwin[0],velwin[0]), freqwin[1]-freqwin[0],
                         velwin[1]-velwin[0], linewidth=1, edgecolor='white',
        ...
        #ax.set_aspect('equal')
        plt.savefig("example_kernel.jpg",bbox_inches='tight',dpi=300,)
        plt.show()
        #%%
